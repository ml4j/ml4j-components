...
/**
 * Default implementation of an activation from a DefaultDirectedComponentBatch
 * instance.
 * 
 * @author Michael Lavelle
 *
 */
public class DefaultDirectedComponentBatchActivationImpl extends DirectedComponentBatchActivationBase
		implements DefaultDirectedComponentBatchActivation {

	/**
	 * @param activations The list of DefaultDirectedComponentChainActivation
	 *                    instances generated by the
	 *                    DefaultDirectedComponentChainBatch
	 */
	public DefaultDirectedComponentBatchActivationImpl(List<DefaultChainableDirectedComponentActivation> activations) {
		super(activations);
	}
	...
	public DirectedComponentGradient<List<NeuronsActivation>> backPropagate(
		...
		for (DefaultChainableDirectedComponentActivation activation : activations) {
			DirectedComponentGradient<NeuronsActivation> grad = new DirectedComponentGradientImpl<>(
					outerGradient.getOutput().get(index));
		...
		for (GradientIndex backPropGrad : activationGradients.parallelStream()
				.map(a -> new GradientIndex(a.getActivation().backPropagate(a.getGradient()), a.getIndex()))
			...
			List<Supplier<AxonsGradient>> backPropAxonsGradients = backPropGrad.getGradient()
	...
	private class ActivationGradientIndex {
		...
		private DefaultChainableDirectedComponentActivation activation;
		private DirectedComponentGradient<NeuronsActivation> gradient;
		private int index;

		public ActivationGradientIndex(DefaultChainableDirectedComponentActivation activation2,
				DirectedComponentGradient<NeuronsActivation> gradient, int index) {
		...
		public DefaultChainableDirectedComponentActivation getActivation() {
			return activation;
		}
