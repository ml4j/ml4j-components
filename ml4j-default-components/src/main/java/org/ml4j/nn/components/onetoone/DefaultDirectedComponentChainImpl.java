 ...
 */
public class DefaultDirectedComponentChainImpl extends DefaultDirectedComponentChainBase implements DefaultDirectedComponentChain {
	...
	public DefaultDirectedComponentChainActivation forwardPropagate(NeuronsActivation neuronsActivation,
		...
		return new DefaultDirectedComponentChainActivationImpl(this, activations, inFlightActivation);
	}


	@Override
	public DefaultDirectedComponentChainImpl dup() {
		return new DefaultDirectedComponentChainImpl(sequentialComponents.stream().map(c -> c.dup()).collect(Collectors.toList()));
	}
