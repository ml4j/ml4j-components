/*
c * Copyright 2019 the original author or authors.
 *
 ...
 */
public class DefaultOneToManyDirectedComponentActivation extends OneToManyDirectedComponentActivationBase implements OneToManyDirectedComponentActivation {

	private static final Logger LOGGER = LoggerFactory.getLogger(DefaultOneToManyDirectedComponentActivation.class);
	 ...
	 */
	public DefaultOneToManyDirectedComponentActivation(MatrixFactory matrixFactory, NeuronsActivation input, int outputNeuronsActivationCount) {
	...
	public DirectedComponentGradient<NeuronsActivation> backPropagate(
		...
		if (allImage) {
			...
			totalActivation = new NeuronsActivationImpl(matrixFactory.createMatrix(gradients.get(0).getActivations(matrixFactory).getRows(), 
					gradients.get(0).getActivations(matrixFactory).getColumns()), gradients.get(0).getFeatureOrientation(), false);
		}
