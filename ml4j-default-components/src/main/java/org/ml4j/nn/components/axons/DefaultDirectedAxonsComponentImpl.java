...
public class DefaultDirectedAxonsComponentImpl<L extends Neurons, R extends Neurons> extends DirectedAxonsComponentBase<L, R, Axons<? extends L, ? extends R, ?>> implements DirectedAxonsComponent<L, R> {
	...
	public DirectedAxonsComponentActivation forwardPropagate(NeuronsActivation neuronsActivation, AxonsContext axonsContext) {
		...
		if (neuronsActivation.getFeatureCount() != getInputNeurons().getNeuronCountExcludingBias()) {
			throw new IllegalArgumentException(getInputNeurons().toString());
		}
		
		AxonsActivation axonsActivation = axons.pushLeftToRight(neuronsActivation, null, axonsContext);
		
		return new DefaultDirectedAxonsComponentActivationImpl(axonsContext, this, axonsActivation);
	}
