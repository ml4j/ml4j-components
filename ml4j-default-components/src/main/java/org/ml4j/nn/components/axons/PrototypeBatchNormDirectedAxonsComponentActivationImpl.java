...
public class PrototypeBatchNormDirectedAxonsComponentActivationImpl<N extends Neurons> extends DirectedAxonsComponentActivationBase<ScaleAndShiftAxons<N>> implements DirectedAxonsComponentActivation {
	 ...
	 */
	public PrototypeBatchNormDirectedAxonsComponentActivationImpl(BatchNormDirectedAxonsComponent<N, ScaleAndShiftAxons<N>> batchNormAxonsComponent,
	...
	public DirectedComponentGradient<NeuronsActivation> backPropagate(
		...
		Matrix exponentiallyWeightedAverageMean = batchNormDirectedAxonsComponent.getExponentiallyWeightedAverageInputFeatureMeans();
		...
		try (InterrimMatrix varianceColumnVectorMul1MinusBeta = varianceColumnVector.mul(1 - (float)beta).asInterrimMatrix()) {
			...
			batchNormDirectedAxonsComponent.setExponentiallyWeightedAverageInputFeatureMeans(exponentiallyWeightedAverageMean);
			...
			batchNormDirectedAxonsComponent.setExponentiallyWeightedAverageInputFeatureVariances(exponentiallyWeightedAverageVariance);
			}
		}
		
		Matrix xhat = leftToRightAxonsActivation.getPostDropoutInput().getActivations(axonsContext.getMatrixFactory());
		...
		if (axonsContext.getRegularisationLambda() != 0) {
			...
			try (InterrimMatrix connectionWeightsCopy = directedAxonsComponent.getAxons().getScaleColumnVector().asInterrimMatrix()) {
		...
		if (axonsContext.getRegularisationLambda() != 0) {
			...
			//LOGGER.debug("Calculating total regularisation Gradients");
				...
				try (InterrimMatrix biasesCopy = directedAxonsComponent.getAxons().getShiftColumnVector().dup().asInterrimMatrix()) {
	...
	public float getTotalRegularisationCost() {
		...
		if (axonsContext.getRegularisationLambda() != 0) {
			...
			if (directedAxonsComponent.getAxons() instanceof TrainableAxons) {
				try (InterrimMatrix weightsWithoutBiases = directedAxonsComponent.getAxons().getDetachedAxonWeights().getConnectionWeights().asInterrimMatrix()) {
					try (InterrimMatrix biases = directedAxonsComponent.getAxons().getDetachedAxonWeights().getLeftToRightBiases().asInterrimMatrix()) {
	...
	@Override
	protected DirectedComponentGradientImpl<NeuronsActivation> createBackPropagatedGradient(AxonsActivation rightToLeftGradientActivation,
		...
		Matrix xhat = leftToRightAxonsActivation.getPostDropoutInput().getActivations(axonsContext.getMatrixFactory());
		Matrix dout = rightToLeftGradientActivation.getPostDropoutOutput().getActivations(axonsContext.getMatrixFactory());
