...
public abstract class DefaultDirectedComponentChainBatchTestBase extends TestBase {
	...
	public void setup() {
	    ...
	    Mockito.when(mockComponent1.getOutputNeurons()).thenReturn(new Neurons1D(300, false));
	    Mockito.when(mockComponent2.getOutputNeurons()).thenReturn(new Neurons1D(400, false));
